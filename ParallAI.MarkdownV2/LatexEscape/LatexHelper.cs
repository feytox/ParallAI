...
public static partial class LatexHelper
{
    ...
    private static string Parse(string latex)
    {
        ...
        while (i < len)
        {
            ...
            // --- ИЗМЕНЕНИЕ ЗДЕСЬ: Убраны блоки else if (c == '{') и else if (c == '}') ---
            // Теперь скобки обрабатываются только если их запросила команда (через ParseBlock)
            // В обычном тексте они остаются символами.
            
            else if (c == '_' || c == '^')
            {
                var isSub = c == '_';
                
                // Проверка на Markdown курсив (пропускаем _)
                if (isSub)
                {
            ...
            {
                // Любой другой символ (включая { и }) просто добавляем
                result.Append(c);
                i++;
            }
        }
    ...
    // ВАЖНО: Обновленный ParseBlock с защитой от незакрытых скобок
    private static (string Content, int NextIndex) ParseBlock(string latex, int start)
    {
        ...
        // Если это не блок {...}, читаем как одиночный аргумент
        ...
        // ЗАЩИТА ОТ НЕЗАКРЫТЫХ СКОБОК
        // Если вышли из цикла по длине строки (pos == Length), но level > 0
        int contentEnd = pos - 1; // Указывает на закрывающую }
        if (level > 0) 
        {
            // Скобка не закрыта! Берем всё до конца строки
            contentEnd = pos;
        }
    ...
    // ... (Остальные методы HandleCommand, MakeNot и т.д. без изменений) ...
    // Вставь сюда код HandleCommand из предыдущих ответов, он не менялся.
       ...
       switch (command)
       {
           ...
           case "\\sqrt":
               ...
               if (cur < latex.Length && latex[cur] == '[') {
                   ...
                   if (close != -1) { deg = latex.Substring(cur + 1, close - cur - 1); cur = close + 1; }
               }
    ...
    // ... Helper методы MakeFraction, MakeSqrt и другие из прошлых ответов ...
    ...
    {
        n = n.Trim(); d = d.Trim();
        return LatexDefinitions.FracMap.TryGetValue((n, d), out var f) ? f : $"{MaybeParenthesize(n)}/{MaybeParenthesize(d)}";
    }
    private static string MakeSqrt(string d, string r) => (d switch { "3" => "∛", "4" => "∜", "" or "2" => "√", _ => $"^{d}√" }) + $"({r})";
    private static string MaybeParenthesize(string t) => t.All(c => char.IsLetterOrDigit(c) || IsCombiningChar(c)) ? t : $"({t})";
    ...
    private static string TranslateCombining(string c, string t) {
        ...
        return i.Type switch { CombiningType.FirstChar => t.Insert(1, i.Char), CombiningType.LastChar => t + i.Char, CombiningType.EveryChar => string.Join("", t.Select(x => x + i.Char)), _ => t };
    }
